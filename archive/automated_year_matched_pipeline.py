#!/usr/bin/env python3
"""
AUTOMATED YEAR-MATCHED PIPELINE

Downloads the year-matched embeddings CSV from Google Drive and aggregates it.
Uses your existing Drive API credentials.

Usage:
    python automated_year_matched_pipeline.py Acer_rubrum
"""
...
def download_year_matched_csv(drive_service, species_name):
    """Download year-matched embeddings CSV from Google Drive"""
...
def aggregate_year_matched_embeddings(csv_path):
    """Aggregate year-matched embeddings to 256D signature"""
...
def main():
    """Main execution"""
    if len(sys.argv) < 2:
        print("Usage: python automated_year_matched_pipeline.py <species_name>")
        print("\nExample:")
        print("  python automated_year_matched_pipeline.py 'Acer rubrum'")
        sys.exit(1)

    species_name = sys.argv[1]

    print("="*70)
    print("AUTOMATED YEAR-MATCHED PIPELINE")
    print("="*70)
    print(f"\nSpecies: {species_name}")
    ...
    # Download CSV
    csv_path = download_year_matched_csv(drive_service, species_name)
    if not csv_path:
        print("\n❌ Download failed")
        sys.exit(1)

    # Aggregate
    signature_df = aggregate_year_matched_embeddings(csv_path)
