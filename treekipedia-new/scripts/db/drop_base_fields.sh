...
# Script to drop legacy base fields (without migrating data) while keeping the researched flag
# This script will remove base fields without migrating any data
...
# Apply the migration
echo -e "${YELLOW}Dropping base fields (no data migration, keeping researched flag)...${NC}"
echo -e "${RED}WARNING: This will DELETE data in base fields without migrating it!${NC}"
echo -e "${RED}Press CTRL+C to cancel or wait 5 seconds to continue...${NC}"
sleep 5
...
if [ $? -eq 0 ]; then
  echo -e "${GREEN}Base fields dropped successfully!${NC}"
else
  echo -e "${RED}Error dropping base fields${NC}"
...
echo -e "${YELLOW}NOTE: Base field data has been DELETED per your instructions.${NC}"